...
import { cardsModel, userModel } from "../../models/index";
...
describe("Board", () => {
  ...
  describe("/POST/board", () => {
    it("should create a personal board - success", done => {
      ...
      request(server).post(boardsUrl).send(organization).end((err, res) => {
  ...
  describe("/POST/boardstars", () => {
    ...
    it("should star a user board - fail", done => {
      request(server).post(`${boardsUrl}null/boardStars`).end((err, res) => {
        assert.equal(
          res.status,
          "400",
  ...
  describe("/PUT/card", () => {
    it("should create a board card - fail", done => {
      request(server).put(`${boardsUrl}${boardId}`).end((err, res) => {
        assert.equal(
          res.status,
          "400",
          "status equals 400 because new card is missing"
        );
  ...
  describe("/GET/card", () => {
    it("should get user board cards - success", done => {
      request(server).get(`${boardsUrl}${boardId}/cards`).end((err, res) => {
    ...
    it("should get user board cards - fail", done => {
      request(server).get(`${boardsUrl}null`).end((err, res) => {
        assert.equal(
          res.status,
          "404",
          "status equals 400 because board id is not valid"
  ...
  describe("/DELETE/board", () => {
    it("should delete a user board - success", done => {
      request(server).delete(`${boardsUrl}${boardId}`).end((err, res) => {
        assert.equal(res.status, "200", "status equals 200");
