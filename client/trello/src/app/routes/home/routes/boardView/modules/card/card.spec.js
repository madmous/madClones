...
describe("card actions", () => {
  ...
  xit("should create an action to getCards - success", () => {
    ...
    nock(`${trelloUrl}`, {
      ...
      .get(`/api/${pathName}/cards`)
      .reply(200, { data });

    return store.dispatch(cardActions.getCards(pathName)).then(() => {
  ...
  it("should create an action to saveCard - success", () => {
    ...
    return store.dispatch(cardActions.saveCard(pathName, cardName)).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
    });
  });
  ...
  it("should create an action to saveCardItem - success", () => {
    ...
    nock(
      `${trelloUrl}api${pathName}/cards/${cardId}`,
      {
        body: JSON.stringify({
          name: cardItemName
        })
    ...
    return store
      .dispatch(cardActions.saveCardItem(pathName, cardId, cardItemName))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  });
