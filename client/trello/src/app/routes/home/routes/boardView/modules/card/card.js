...
export function moveCardItemAndUpdateCards(
  previousAndNextPositions,
  cards,
  pathname
) {
  return dispatch => {
    ...
    return request(`${trelloUrl}api${pathname}/cards/`, {
      method: "PUT",
      body: JSON.stringify({
        cards
      }),
      ...
      response => {
        dispatch(updateCardsFail(response));
      }
    );
  };
}
  ...
  return dispatch => {
    ...
    return request(`${trelloUrl}api${pathname}/cards/`, {
      ...
      response => {
        ...
        dispatch(updateCards(responseData));
      },
      response => {
        dispatch(loadCardsFail(response));
...
export function saveCard(pathName, cardName) {
  return dispatch => {
    ...
    return request(`${trelloUrl}api${pathName}/cards/`, {
      ...
      response => {
        dispatch(saveCardFail(response));
...
export function saveCardItem(pathname, cardId, cardItemName) {
  return dispatch => {
    ...
    return request(`${trelloUrl}api${pathname}/cards/${cardId}`, {
      method: "POST",
      body: JSON.stringify({
        name: cardItemName
      }),
      ...
      response => {
        dispatch(saveCardItemFail(response));
...
export default function boardView(state = initialState, action) {
  switch (action.type) {
    ...
    case MOVE_CARD: {
      ...
      if (lastX === nextX) {
        newCards[lastX].cardItems.splice(
          nextY,
          0,
          newCards[lastX].cardItems.splice(lastY, 1)[0]
        );
      } else {
        newCards[nextX].cardItems.splice(
          nextY,
          0,
          newCards[lastX].cardItems[lastY]
        );
